...
const GameRoom: React.FC<GameRoomProps> = ({ onLeave, boardNumber, stake, balance, setBalance, roomId, isDarkMode }) => {
  ...
  const handleBingoClick = () => {
    ...
    if (win) {
      ...
      alert("No Bingo pattern detected!");
    }
  };
  ...
  return (
    <div className={`flex flex-col h-full ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#a28cd1]'} overflow-hidden animate-fadeIn select-none`}>
      <div className="grid grid-cols-7 gap-1 px-2 py-2 shrink-0">
        <div className="bg-white/10 rounded p-1 text-center text-[8px] text-white">Pot: {DERASH.toFixed(0)}</div>
      </div>

      <div className="flex flex-1 overflow-hidden px-2 gap-2 pb-1 pt-1">
        <div className="w-[150px] flex flex-col bg-white/20 rounded-xl overflow-hidden p-1 shrink-0">
          <div className="grid grid-cols-5 gap-0.5 mb-1">
            {letters.map((l, i) => <div key={i} className={`${l.color} text-white text-[10px] font-black text-center py-1.5`}>{l.label}</div>)}
          </div>
          <div className="flex-1 grid grid-cols-5 gap-0.5">
            {Array.from({ length: 15 }).map((_, rowIdx) => boardCols.map((col, colIdx) => {
              ...
              return <div key={`${colIdx}-${rowIdx}`} className={`flex items-center justify-center rounded-sm text-[9px] font-black ${isCalled ? 'bg-white text-[#5e35b1]' : 'bg-white/10 text-white/40'}`}>{num}</div>;
            }))}
          </div>
        </div>

        <div className="flex-1 flex flex-col gap-1.5 overflow-hidden">
          <div className="bg-gray-900 rounded-lg p-2 flex justify-between items-center h-14">
            <span className="text-[10px] font-black text-white uppercase">Call</span>
            <div className="w-10 h-10 bg-[#f48120] rounded-full flex items-center justify-center text-white text-xl font-black">{currentCall || '-'}</div>
          </div>
          <div className="bg-white/20 rounded-xl p-1.5 flex-1 flex flex-col items-center justify-center relative">
            <div className="grid grid-cols-5 gap-1 w-full aspect-square max-w-[190px]">
              {cardMatrix.map((row, rIdx) => row.map((val, cIdx) => {
                ...
                return <button key={`${cIdx}-${rIdx}`} onClick={() => handleMark(val)} className={`aspect-square flex items-center justify-center rounded-sm font-black text-base border border-black/5 ${val === '*' ? 'bg-green-700 text-white' : isMarked ? (isTrulyCalled ? 'bg-green-600 text-white' : 'bg-gray-700 text-white/70') : 'bg-white/90 text-black'}`}>{val === '*' ? '★' : val}</button>;
              }))}
            </div>
          </div>
        </div>
      ...
      <div className="px-3 py-3 space-y-2 shrink-0 bg-black/10 backdrop-blur-md">
        <button onClick={handleBingoClick} disabled={gameState !== 'running'} className={`w-full py-4 rounded-full font-black text-2xl shadow-xl uppercase ${gameState !== 'running' ? 'bg-orange-500/50 text-white/50' : 'bg-[#f48120] text-white'}`}>BINGO!</button>
        <button onClick={onLeave} className="w-full bg-[#f04e4e] text-white py-2.5 rounded-full font-black text-[11px] uppercase">Leave</button>
      </div>
      ...
      {gameState === 'ended' && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-[#a28cd1] rounded-[2.5rem] p-8 text-center shadow-2xl max-w-[300px] w-full border border-white/20 text-white">
            <h2 className="text-2xl font-black mb-1 uppercase">GAME OVER</h2>
            <p className="text-4xl font-black text-yellow-400 mb-6">{winInfo ? DERASH.toFixed(0) : 0} ETB</p>
            <button onClick={onLeave} className="w-full py-4 bg-white text-[#a28cd1] rounded-full font-black uppercase">Return</button>
          </div>
        </div>
      )}
    </div>
